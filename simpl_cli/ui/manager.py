...
class UIManager:
    ...
    def get_style(self) -> Style:
        ...
        return merge_styles([default_ui_style(), default_pygments_style(), custom_style])
    ...
    def display_memory_status(self, stats: dict) -> None:
        ...
        self.console.print(
            Panel.fit(status_table, title="󰍛 Memory Status", border_style="blue")
        )
    ...
    def display_file_explorer(
        ...
        dir_table = Table(title="Directories", show_header=True, header_style="bold cyan")
        ...
        tables = [PanelTheme.build(dir_table, title=f" Directories ({dir_total})", style="info", fit=True)]
        tables.append(PanelTheme.build(file_table, title=f" Files ({file_total})", style="info", fit=True))

        if preview:
            preview_title = os.path.basename(preview.get("path", base_path)) if preview.get("path") else "Preview"
        ...
        self.console.print(PanelTheme.build(summary, style="default", title="File Explorer", fit=True))
    ...
    def display_command_not_found(
        ...
        tips_node.add("• Double-check for typos or run 'which <command>' to verify availability")
        tips_node.add("• Need AI assistance? Press Ctrl+A to switch into AI mode before asking")
    ...
    def _build_preview_renderable(self, preview: dict):
        ...
        if language and language != "text":
            try:
                return Syntax(content, language, theme="vim", line_numbers=True, indent_guides=True)
            except Exception:
