...
class EnvironmentDetector:
    ...
    def get_python_environment(self) -> Optional[Dict[str, str]]:
        if not self._should_refresh_cache():
            return self._cache.get('python_env')
    ...
    def get_git_status(self) -> Optional[Dict[str, str]]:
        cwd = os.getcwd()
        cached_path = self._cache.get('_cwd_git_status')
        if (
            cached_path
            and cwd.startswith(cached_path)
            and not self._should_refresh_cache()
        ):
            return self._cache.get('git_status')
        ...
        self._cache['_cwd_git_status'] = cwd if git_info else None
        self._update_cache_time()
        return git_info
    ...
    def get_node_environment(self) -> Optional[Dict[str, str]]:
        if not self._should_refresh_cache():
            return self._cache.get('node_env')
